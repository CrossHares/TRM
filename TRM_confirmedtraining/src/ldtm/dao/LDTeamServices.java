...
JdbcTemplate template = (JdbcTemplate) new ClassPathXmlApplicationContext("spring-config.xml").getBean("jt");
	...
	public List<LDTeam> showAllTrainingMemberService()
	{
		...
		List<LDTeam> list = template.query("SELECT * FROM LD_TRAINING_MEMBER", new LDTeamMapper());
		return list;
	}
	...
	//get ldtm id from a name
	public int getIDfromName(String name){
		int ldt = template.queryForInt("SELECT LDTM_ID FROM LD_TRAINING_MEMBER WHERE LDTM_NAME = ?", new Object[]{name});
		return ldt;
	}
}
